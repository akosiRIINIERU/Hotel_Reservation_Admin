...
export default function ManageRooms() {
  ...
  // Expanded state to include all your new requirements
        ...
        <form onSubmit={addRoom} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
            <div style={{ flex: 1 }}>
              ...
              <input required type="number" value={newRoom.capacity} onChange={e => setNewRoom({...newRoom, capacity: e.target.value})} placeholder="People" />
            </div>
            <div style={{ flex: 1 }}>
              <label>Beds</label>
              <input required type="number" value={newRoom.beds} onChange={e => setNewRoom({...newRoom, beds: e.target.value})} placeholder="Count" />
            </div>
            <div style={{ flex: 1 }}>
              <label>Toilets</label>
              <input required type="number" value={newRoom.toilets} onChange={e => setNewRoom({...newRoom, toilets: e.target.value})} placeholder="Count" />
            </div>
          </div>
          ...
          <div>
            ...
            <textarea 
              style={{ width: '100%', background: 'var(--bg-dark)', color: 'white', border: '1px solid var(--border-color)', padding: '12px', borderRadius: '4px', minHeight: '100px', fontFamily: 'inherit' }}
              value={newRoom.description}
              onChange={e => setNewRoom({...newRoom, description: e.target.value})}
              placeholder="Describe the room features (WiFi, View, Mini-bar, etc.)"
            />
          </div>
          ...
          <div>
            ...
            <label htmlFor="image-upload" style={{ display: 'flex', justifyContent: 'center', padding: '30px', border: '2px dashed var(--border-color)', borderRadius: '4px', cursor: 'pointer', backgroundColor: imageFile ? 'rgba(212, 175, 55, 0.1)' : 'transparent', borderColor: imageFile ? 'var(--gold-accent)' : 'var(--border-color)' }}>
              <span style={{ color: imageFile ? 'var(--gold-accent)' : 'var(--text-muted)' }}>
                {imageFile ? `✅ Ready: ${imageFile.name}` : 'Click to Upload Image'}
              </span>
            </label>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '30px' }}>
        {rooms.map(room => (
          <div key={room.id} className="room-card" style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <div className="room-details" style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: '15px', margin: '15px 0', fontSize: '0.85rem', color: 'var(--text-main)' }}>
                ...
                <span>🚿 {room.toilets}</span>
              </div>

              <p style={{ fontSize: '0.85rem', lineHeight: '1.4', color: 'var(--text-muted)', minHeight: '40px' }}>
                {room.description || 'No description provided.'}
              </p>
              
              <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
                <button onClick={() => toggleStatus(room.id, room.is_available)} style={{ flex: 2, fontSize: '0.75rem', borderColor: room.is_available ? '#ff4444' : 'var(--gold-accent)', color: room.is_available ? '#ff4444' : 'var(--gold-accent)' }}>
